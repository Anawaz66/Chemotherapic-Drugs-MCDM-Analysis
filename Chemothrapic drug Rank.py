...
e_values = -1 / np.log(8) * np.sum(N * np.log(N), axis=1)
...
# Original data
data = {
    'Drugs': ['Daunorubicin', 'Dacarbazine', 'Bendamustine', 'Dactinomycin',
              'Ifosfamide', 'Methotrexate', 'Doxorubicin', 'Capecitabine'],
    'Copras': [5, 2, 7, 1, 3, 6, 4, 8],
    'Topsis': [3, 8, 6, 1, 7, 4, 2, 5]
}
df = pd.DataFrame(data)
...
ax.set_xlabel('Drugs (sorted by COPRAS rank)', fontsize=12, fontweight='bold')
ax.set_ylabel('Rank (lower is better)', fontsize=12, fontweight='bold')
ax.set_title('Ranking of Drugs by COPRAS and TOPSIS (sorted by COPRAS)', fontsize=14, fontweight='bold')
ax.set_xticks(x)
